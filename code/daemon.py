...
def init_art_tracking():
	...
	_art.stations[11] = avango.daemon.Station('pointer')
...
def init_pointer():
	_string = os.popen(
		"/opt/avango/vr_application_lib/tools/list-ev -s | grep \"MOUSE USB MOUSE\" | sed -e \'s/\"//g\'  | cut -d\" \" -f4").read()

	if len(_string) == 0:
		_string = os.popen(
			"/opt/avango/vr_application_lib/tools/list-ev -s | grep \"MOSART Semi. Input Device\" | sed -e \'s/\"//g\'  | cut -d\" \" -f4").read()  # search for other pointer
	...
	if len(_string) > 0:
		...
		_pointer.buttons[0] = "EV_KEY::KEY_F5"
		_pointer.buttons[1] = "EV_KEY::KEY_PAGEUP"
		_pointer.buttons[2] = "EV_KEY::KEY_PAGEDOWN"
